...
public class PressureSystemManager : MonoBehaviour
{
    ...
    [SerializeField] private float pressureIncreaseRate = 2.0f; 
    ...
    [SerializeField] private GameObject handheldDeviceUI; // B'ye basınca açılacak UI paneli
    [SerializeField] private TextMeshProUGUI pressureText; // Paneldeki yazı

    [Header("Game Over / Warning")]
    [SerializeField] private GameObject explosionEffect; // %100 olunca çıkacak efekt
    [SerializeField] private GameObject warningUI; // %90 üstü uyarı ikonu/yazısı
    ...
    private void Start()
    {
        if (handheldDeviceUI != null) handheldDeviceUI.SetActive(false);
    ...
    private void Update()
    {
        ...
        {
            if (warningUI != null && !warningUI.activeSelf) warningUI.SetActive(true);
            // Buraya alarm sesi çalma kodu eklenebilir.
        }
        else
        {
            if (warningUI != null && warningUI.activeSelf) warningUI.SetActive(false);
        }
