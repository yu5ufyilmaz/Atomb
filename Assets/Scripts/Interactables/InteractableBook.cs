...
public class InteractableBook : MonoBehaviour, IInteractable
{
    ...
    [Tooltip(
        "Buradaki slider'ları değiştirerek yandaki Scene penceresinde Gizmo'yu canlı olarak ayarlayın."
    )]
    [SerializeField]
    [Range(0, 1)]
    ...
    [Tooltip(
        "Ayarlamayı bitirdikten sonra buradaki Rect değerlerini kopyalayıp ScriptableObject'e yapıştırın."
    )]
    [SerializeField]
    ...
    [Tooltip(
        "Tek bir sayfanın modelinizin local space'indeki fiziksel boyutu (Genişlik, Yükseklik)"
    )]
    [SerializeField]
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError(
                $"{gameObject.name}: Kitap sayfası materyali (bookPagesMaterial) bulunamadı!",
                this
            );
        }
    ...
    private void Start()
    {
        ...
        {
            Debug.LogWarning(
                $"InteractableBook ({gameObject.name}): 'Interaction Collider' (BoxCollider) Inspector'dan atanmamış veya obje üzerinde bulunamadı!",
                this
            );
        }
        ...
        {
            Debug.LogWarning(
                $"InteractableBook ({gameObject.name}): 'Player Look Script' atanmamış. Kitap açılınca fare (kamera) hareketini tam olarak durdurmak için, oyuncunun 'MouseLook', 'ThirdPersonController' veya 'StarterAssetsInputs' gibi script'ini Inspector'dan bu alana sürüklemeniz önerilir."
            );
        }
        ...
        if (cameraTransform == null)
        {
            ...
            {
                Debug.LogError(
                    "Ana kamera (Main Camera) bulunamadı! Lütfen 'cameraTransform' değişkenini Inspector'dan atayın."
                );
            }
        }
        ...
        {
            Debug.LogError(
                "InteractableBook scripti, şifre tıklaması için bir Collider bileşenine ihtiyaç duyuyor!",
                this
            );
        }
    ...
    private void CheckForPasswordClick()
    {
        ...
        {
            Debug.Log(
                "-> Tıklama algılandı, ancak açık olan sayfalar ({pageIndexL}-{pageIndexR}) şifre sayfası ({passwordPage}) değil. İşlem durduruldu."
            );
            return;
        }
        ...
        {
            Debug.Log(
                $"IŞIN ÇARPTI: Obje = {hit.collider.gameObject.name}, Layer = {LayerMask.LayerToName(hit.collider.gameObject.layer)}"
            );

            if (hit.collider == bookCollider)
            {
                Debug.Log(
                    "-> BAŞARILI: Işın kitabın kendi collider'ına ('{bookCollider.name}') çarptı. UV koordinatları alınıyor..."
                );
                ...
                else
                {
                    ...
                    Debug.Log(
                        "   -> Kontrol: Tıklanan sayfa (Sol/Sağ) beklenen şifre sayfasıyla ({passwordPage}) eşleşmedi."
                    );
                }
            }
            else
            {
                ...
                Debug.Log(
                    "-> HATA: Işın bir şeye çarptı, ama bu objenin beklenen collider'ı ('{bookCollider.name}') DEĞİLDİ."
                );
            }
        }
        else
    ...
    public void AssignPassword(PasswordData data)
    {
        ...
        Debug.Log(
            $"{gameObject.name} kitabına {passwordID} şifresi atandı. YENİ Toplam Sayfa: {this.totalPages}"
        );
    }
